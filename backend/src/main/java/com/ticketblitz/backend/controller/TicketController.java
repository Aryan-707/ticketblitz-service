...
public class TicketController {
    ...
    public ResponseEntity<String> buyTicket(@RequestBody PurchaseRequest request, Principal principal) {
        ...
        if (reqCount != null && reqCount == 1) {
            redisTemplate.expire(rateLimitKey, 1, java.util.concurrent.TimeUnit.MINUTES);
        }
        if (reqCount != null && reqCount > 5) {
        ...
        try {
            log.info("Processing purchase request: User={}, Event={}, Tier={}, Seat={}, IdempotencyKey={}",
                    userId, request.getEventId(), request.getTierId(), request.getSeatId(), request.getIdempotencyKey());
            ...
            if (success) {
                ...
                return ResponseEntity.ok("Successfully secured seat " + request.getSeatId() + "!");
            } else {
                log.warn("Purchase FAILED for User: {} - Stock depleted or limit exceeded.", userId);
                return ResponseEntity.status(409).body("Booking failed: Seat or Tier no longer available.");
            }
        } finally {
            MDC.clear();
        }
